...
def extra_practice(directory=None):
    ...
    original_ball = image_list[0]
    basketball_mask = PIL.Image.new('RGBA', original_ball.size, (127,0,127,0))
    drawing_layer = PIL.ImageDraw.Draw(basketball_mask)
    drawing_layer.ellipse((100,100, 2*295, 2*295), fill=(0,127,127,255))
    basketball_mask.save('zmask.png')
    result = PIL.Image.new('RGBA', basketball_mask.size, (0,0,0,0))
    result.paste(original_ball, (0,0), mask=basketball_mask)  
    result.save('z.png')
    
    img1 = np.array(result)
    
    height = result.size[0]
    width = result.size[1]

    for r in range(600):
        for c in range(600):
            if sum(img1[r][c]) - 255 < 200:
                img1[r][c]=[255,255,255,255]
